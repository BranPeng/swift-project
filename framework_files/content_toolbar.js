 ...
 * 二级域名 getSecondaryDomain
  ...
  //  festival customize
    ...
    var searchWordArr = ['疯狂Python精讲', 'Python工程师'],
        searchWordN = Math.round(Math.random()),
        searchWord = ''
    storage = window.localStorage;
    if(!storage.getItem('randomSearchWord')){
      storage.setItem("randomSearchWord",searchWordN);
    }
    if(storage.getItem('randomSearchWord') === '0'){
      searchWord = searchWordArr[0]
      storage.setItem("randomSearchWord", '1');
    } else {
      searchWord = searchWordArr[1]
      storage.setItem("randomSearchWord", '0');
    }
